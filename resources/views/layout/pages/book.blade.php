        ...
        <div class="row m-0">
            ...
            <div class="item-details col-12 col-md-8">
                <div class="item-details-top">
                    ...
                    <div class="book-info">{{$book->author}},
                        <time datetime="{{$book->publish_date}}"
                              title="{{$book->publish_date}}">{{date('d.m.Y', strtotime($book->publish_date))}}</time>
                    </div>
                ...
                <form action="{{ route('cart.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="number" id="quantity" name="quantity" class="form-control input-number input-quantity"
                           value="{{ is_null($book->quantity) ?? 1 }}" min="1" max="100">
                </form>
            </div>
        </div>
    </div>
